        ...
        : TKSingleBufferedBlobStorageBase<LatestEndpointRequestsHistory, EndpointControlEndpointRequestData>, IEndpointControlRequestHistoryStorage
{
    ...
    private readonly TKEpiserverBlobHelper<LatestEndpointRequestsHistory> _blobHelper;

    /// <summary>
    /// Stores request data in episerver blobstorage.
    /// </summary>
    public TKEpiserverBlobEndpointControlRequestHistoryStorage(IBlobFactory blobFactory, ITKCache cache)
        : base(cache)
    {
        BlobUpdateBufferDuration = TimeSpan.FromSeconds(30);
        MaxBufferSize = 2500;
        _blobHelper = new TKEpiserverBlobHelper<LatestEndpointRequestsHistory>(blobFactory, () => ContainerIdWithFallback, () => ProviderName);
    }
