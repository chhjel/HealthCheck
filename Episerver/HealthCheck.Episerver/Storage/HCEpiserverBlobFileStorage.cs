...
namespace HealthCheck.Episerver.Storage
{
    ...
    /// <para>Files will be stored in a single container (defaults to hardcoded 88888888-ff7a-40d6-a41f-cdbc654e28f1) with $"{Guid.NewGuid()}.HCUpload" as blob names.</para>
    /// </summary>
    public class HCEpiserverBlobFileStorage : ISecureFileDownloadFileStorage
    {
        /// <summary>
        /// Id of this storage.
        /// </summary>
        ...
        public HCEpiserverBlobFileStorage(IBlobFactory blobFactory)
        {
        ...
        public virtual IEnumerable<HCSecureFileDownloadFileDetails> GetFileIdOptions()
            => Enumerable.Empty<HCSecureFileDownloadFileDetails>();

        /// <inheritdoc />
        public Task<HCSecureFileDownloadUploadResult> UploadFileAsync(Stream fileStream)
        {
            var fileId = $"{Guid.NewGuid()}.HCUpload";
            var blobId = GetBlobId(fileId);
            var blob = _blobFactory.GetBlob(blobId);

            if (HasFile(fileId))
            {
                return Task.FromResult(new HCSecureFileDownloadUploadResult { ErrorMessage = "A file with this blob id already exists, try again." });
            }

            fileStream.Seek(0, SeekOrigin.Begin);
            blob.Write(fileStream);

            var result = new HCSecureFileDownloadUploadResult
            {
                Success = true,
                FileId = fileId
            };
        ...
        public Task<bool> DeleteUploadedFileAsync(string fileId)
        {
            ...
            {
                HCGlobalConfig.OnExceptionEvent?.Invoke(typeof(HCEpiserverBlobFileStorage), nameof(DeleteUploadedFileAsync), ex);
