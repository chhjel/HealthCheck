...
/// <para>If you need item ids use <see cref="TKSimpleDataStoreWithId{TItem, TId}"/> instead.</para>
/// </summary>
public class TKSimpleDataStore<TItem> : IDataStore<TItem>
{
    /// <summary>
    /// Options for automatic storage cleanup.
    /// </summary>
    public TKStorageRetentionOptions<TItem> RetentionOptions { get; set; }
    ...
    public TKSimpleDataStore(string filepath,
    ...
    public void DeleteWhere(Func<TItem, bool> condition)
    {
        ...
        {
            // Load
            var loadTimer = new TKMetricsTimer($"{GetType().GetFriendlyTypeName()}.LoadData()");
            var linesToKeep = File.ReadLines(FilePath)
                .Where(x => CheckCondition(x, (item) => !condition(item), false));
            TKMetricsContext.AddGlobalTimingValue(loadTimer);

            // Save
            var saveTimer = new TKMetricsTimer($"{GetType().GetFriendlyTypeName()}.SaveData()");
            ...
            TKMetricsContext.AddGlobalTimingValue(saveTimer);
        }
    }
