...
/// </summary>
public class TKRequestData
{
    private const string _requestItemKey = "___tk_request_data";
    ...
    /// </summary>
    public List<TKRequestDataErrorDetails> Errors { get; private set; } = new();
    ...
    public static void AddError(string error, Exception ex = null)
        => UpdateCurrentRequestData(x =>
        {
            x.Errors.Add(new TKRequestDataErrorDetails
            {
                Error = error,
                Details = TKExceptionUtils.GetFullExceptionDetails(ex, returnNullIfNull: true)
            });
        });
    ...
    /// <summary>
    /// Retrives the <see cref="TKRequestData"/> object for the current request.
    /// </summary>
    public static TKRequestData GetCurrentRequestData()
    {
        var data = TKRequestContext.GetRequestItem<TKRequestData>(_requestItemKey, null);
        return data ?? new TKRequestData();
    }

    private static void UpdateCurrentRequestData(Action<TKRequestData> action)
    {
    ...
    private static void UpdateCurrentRequestData(TKRequestData data)
        => TKRequestContext.SetRequestItem(_requestItemKey, data);

    /// <summary>
    /// Some error details.
    /// </summary>
    public class TKRequestDataErrorDetails
    {
        /// <summary>
        /// Error message.
        /// </summary>
