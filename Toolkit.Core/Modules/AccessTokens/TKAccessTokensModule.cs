...
/// <summary>
/// Module for managing TK access tokens.
/// </summary>
public class TKAccessTokensModule : ToolkitModuleBase<TKAccessTokensModule.AccessOption>
{
    private TKAccessTokensModuleOptions Options { get; }

    /// <summary>
    /// Module for managing TK related access.
    /// </summary>
    public TKAccessTokensModule(TKAccessTokensModuleOptions options)
    {
        Options = options;
    }
    ...
    /// </summary>
    public override IToolkitModuleConfig GetModuleConfig(ToolkitModuleContext context) => new TKAccessTokensModuleConfig();
    ...
    /// </summary>
    public TKAccessToken GetTokenForRequest<TAccessRole>(RequestInformation<TAccessRole> currentRequestInformation)
    {
        ...
        try
        {
            ...
            var isValid = TKHashUtils.ValidateHash(tokenHashBase, tokenFromStore.HashedToken, tokenFromStore.TokenSalt);
    ...
    /// View all token data.
    /// </summary>
    [ToolkitModuleMethod(AccessOption.ViewToken)]
            ...
            ?.Select(x =>
            {
                ...
                if (x.ExpiresAt != null)
                {
                    ...
                    expirationSummary = (timeUntil < 0)
                        ? "Expired"
                        : $"Expires in {TKTimeUtils.PrettifyDuration(timeUntil)}";
                }

                string lastUsedSummary = null;
                if (x.LastUsedAt != null)
                {
                    lastUsedSummary = $"Last used {TKTimeUtils.PrettifyDurationSince(x.LastUsedAt, TimeSpan.FromMinutes(1), "less than a minute")} ago";
                }
                else
                {
    ...
    /// View all token data.
    /// </summary>
    [ToolkitModuleMethod(AccessOption.DeleteToken)]
    public object DeleteToken(ToolkitModuleContext context, Guid id)
    {
    ...
    public object CreateNewToken(ToolkitModuleContext context, CreateNewTokenRequest data)
    {
        if (Options.TokenStorage == null) return null;
        ...
        var modules = data.Modules
            ...
            .Select(x =>
            {
                ...
                return new TKAccessTokenModuleData()
                {
                    ModuleId = x.ModuleId,
        ...
        var tokenHash = TKHashUtils.GenerateHash(tokenHashBase, out string salt);

        var token = new TKAccessToken
        {
            Id = id,
            Name = data.Name,
    ...
    public object GetAccessData(ToolkitModuleContext context)
    {
        ...
        var moduleOptions = context.LoadedModules
            ...
            .Select(x => {
                ...
                List<TKModuleIdData> ids;
                if (requestModuleIds?.Any() == true)
                {
                    ids = x.AllModuleIds?.Where(mid => requestModuleIds?.Contains(mid.Id) == true)?.ToList() ?? new List<TKModuleIdData>();
                }
                else
                {
                    ids = x.AllModuleIds ?? new List<TKModuleIdData>();
                }
    ...
    /// </summary>
    [ToolkitModuleAction]
    public object ATTokenKillswitch(ToolkitModuleContext context, string url)
    {
    ...
    private string CreateBaseForHash(string rawToken, List<string> roles, List<TKAccessTokenModuleData> modules, DateTimeOffset? expiresAt)
    {
    ...
    private class ModuleAccessData
    {
        ...
        public List<TKModuleIdData> AccessIds { get; set; }
    }
