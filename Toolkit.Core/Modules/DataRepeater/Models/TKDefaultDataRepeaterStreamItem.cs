...
/// <summary>
/// Default item model that can be stored in a <see cref="ITKDataRepeaterStream"/>.
/// <typeparamref name="TData"/> must be serializable. Can be a string or complex object.
/// </summary>
public class TKDefaultDataRepeaterStreamItem<TData, TSelf> : ITKDataRepeaterStreamItem
    where TData : class
    where TSelf : TKDefaultDataRepeaterStreamItem<TData, TSelf>, new()
{
    #region ITKDataRepeaterStreamItem Implementation
    /// <inheritdoc />
    ...
    public TData Data
    {
        get
        {
            ...
            else return TKGlobalConfig.Serializer.Deserialize<TData>(serializedData);
        }
        set
        {
            SerializedData = null;
            if (value != null)
            {
                SerializedData = typeof(TData) == typeof(string)
                    ? value as string
                    : TKGlobalConfig.Serializer.Serialize(value, pretty: true);
            }
        }
    }
    ...
    public TData OriginalData
    {
        get
        {
            ...
            else return TKGlobalConfig.Serializer.Deserialize<TData>(FirstSerializedData);
        }
    }
    ...
    /// <param name="includeTKRequestErrors">If true, any errors in the current <see cref="TKRequestData"/> will be included in error.</param>
    ...
    public static TSelf CreateFrom(TData data, string itemId, string summary = null,
        ...
        bool includeTKRequestErrors = false,
        bool serialize = true)
    {
        ...
        if (serialize && data != null)
        {
            serializedData = typeof(TData) == typeof(string)
                ? data as string
                : TKGlobalConfig.Serializer.Serialize(data, pretty: true);
        }
        ...
        if (exception != null)
        {
            appendError(TKExceptionUtils.GetFullExceptionDetails(exception));
        }

        if (includeTKRequestErrors)
        {
            var requestData = TKRequestData.GetCurrentRequestData();
    ...
    /// </summary>
    public TSelf SetForcedStatus(TKDataRepeaterStreamItemStatus? forcedStatus)
    {
