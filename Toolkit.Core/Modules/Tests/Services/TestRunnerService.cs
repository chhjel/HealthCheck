...
namespace QoDL.Toolkit.Core.Modules.Tests.Services;
...
public class TestRunnerService
{
    ...
    /// </summary>
    public static Func<TKTestContext> GetCurrentTestContext { get; set; }
    ...
    public async Task<List<TestResult>> ExecuteTests(
        ...
        Action<TestResult, object> resultAction = null)
    {
        var iocScope = new TKIoCUtils.ScopeContainer();
        var results = new ConcurrentBag<TestResult>();
        foreach (var testClass in testClasses)
        {
            var classInstance = TKIoCUtils.GetInstanceExt(testClass.ClassType, maxDepth: MaxIoCRecursiveDepth, scope: iocScope);
        ...
        Action<TestResult, object> resultAction = null)
    {
        ...
        try
        {
            ...
            if (test.Type == TestDefinition.TestDefinitionType.Normal)
            {
                ...
                if (instance == null)
                {
                    var instanceResult = TKIoCUtils.GetInstanceExtWithDetails(test.ParentClass.ClassType, maxDepth: MaxIoCRecursiveDepth);
            ...
            else if (test.Type == TestDefinition.TestDefinitionType.ProxyClassMethod)
            {
                ...
                if (instance == null)
                {
                    var instanceResult = TKIoCUtils.GetInstanceExtWithDetails(test.ClassProxyConfig.TargetClassType, maxDepth: MaxIoCRecursiveDepth);
                    instance = instanceResult.Instance;
                    instanceError = instanceResult.ErrorDetails;
                }
            }
        ...
        {
            var extraDetails = TKExceptionUtils.GetExceptionDetails(ex);
