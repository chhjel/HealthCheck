...
namespace HealthCheck.WebApi.Core.Utils
{
    /// <summary>
    /// Utils related to http requests for WebApi.
    /// </summary>
    public static class HCWebApiRequestUtils
    {
        ...
        /// 3. First header with value in prioritized order from <paramref name="headers"/>. If left null <see cref="HCRequestUtils.DefaultClientIPHeaders"/> is used.
        /// </code>
        /// </summary>
        public static string? GetIPAddress(HttpRequestMessage request, bool stripPortNumber = true, Func<HttpRequestMessage?, string?>? customResolver = null, List<string?>? headers = null)
        {
            try
            {
                ...
                headers ??= HCRequestUtils.DefaultClientIPHeaders;
                return HCRequestUtils.GetIPAddress(request, _webApiIpResolvers, stripPortNumber, customResolver, headers);
            }
            catch (Exception)
            {
                return null;
            ...
            try
            {
                ...
                if (request.Properties.ContainsKey(_httpContext))
                {
                    ...
                    if (ctx != null)
                    {
                        ...
                        {
                            return HCRequestUtils.LocalhostValue;
                        }
