...
namespace HealthCheck.Core.Modules.Tests
{
    ...
    public class TestsModule : HealthCheckModuleBase<TestsModule.TestsModuleAccessOption>
    {
        ...
        /// Module for executing tests at runtime.
        /// </summary>
        public TestsModule(TestsModuleOptions options)
        {
            TestDiscoverer.AssemblyContainingTests = options.AssemblyContainingTests
                ?? throw new ArgumentNullException($"[{nameof(TestsModuleOptions)}.{nameof(TestsModuleOptions.AssemblyContainingTests)}] must be set to an assembly to retrieve tests from must be provided.");
        }
        ...
        /// Configure grouping.
        /// </summary>
        public TestsModule ConfigureGroups(Action<TestSetGroupsOptions> optionsModifier)
        {
        ...
        public TestsDataViewModel GetTests(HealthCheckModuleContext context)
        {
            ...
            if (context.HasAccess(TestsModuleAccessOption.ViewInvalidTests))
            {
