...
namespace HealthCheck.Core.Util
{
    ...
    /// <para>Initialize on startup using <c>HCSelfUptimeChecker.EnsureIntervalCheckStarted(..)</c></para>
    /// <para>Any exceptions will be sent to <see cref="HCGlobalConfig.OnExceptionEvent"/></para>
    /// </summary>
    public class HCSelfUptimeChecker
	{
		private static HCSelfUptimeChecker Instance { get; } = new();
		...
		private IHCSelfUptimeCheckerStorage _storage;
		...
		public static void EnsureIntervalCheckStarted(IHCSelfUptimeCheckerStorage storage, TimeSpan checkInterval, TimeSpan failIfNoCheckForDuration, OnBackUpAfterDowntime onDowntimeDetected)
		{
		...
		private async Task IntervalCheckAsync(TimeSpan checkInterval)
		{
			...
			while (!cancel)
			{
				...
				{
					HCGlobalConfig.OnExceptionEvent?.Invoke(typeof(HCSelfUptimeChecker), nameof(IntervalCheckAsync), ex);
				}
			}
		}
