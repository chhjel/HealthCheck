...
{
    public class TKEpiserverBlobDataRepeaterStreamItemStorageTests
    {
        public TKEpiserverBlobDataRepeaterStreamItemStorageTests()
        {
        ...
        public async Task AddItem_DuplicateWithMerge_MergesData()
        {
            ...
            var service = new TKDataRepeaterService(new[] { stream });
        ...
        public async Task SetItemTags_Alt1_WorksAsExpected()
        {
            ...
            async Task<List<ITKDataRepeaterStreamItem>> getItemsAsync()
                => (await storage.GetItemsPagedAsync(new TKGetDataRepeaterStreamItemsFilteredRequest
                { PageIndex = 0, PageSize = int.MaxValue })).Items.ToList();
        ...
        public class StreamImplementation : TKDataRepeaterStreamBase<StreamItem>
        {
            ...
            public StreamImplementation(ITKDataRepeaterStreamItemStorage storage) : base(storage) { }

            protected override Task<TKDataRepeaterStreamItemDetails> GetItemDetailsAsync(StreamItem item) => Task.FromResult<TKDataRepeaterStreamItemDetails>(null);
            protected override Task<TKDataRepeaterItemAnalysisResult> AnalyzeItemAsync(StreamItem item, bool isManualAnalysis = false) => Task.FromResult<TKDataRepeaterItemAnalysisResult>(null);
            protected override Task<TKDataRepeaterRetryResult> RetryItemAsync(StreamItem item) => Task.FromResult<TKDataRepeaterRetryResult>(null);
        }
    }
}
