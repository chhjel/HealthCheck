...
namespace HealthCheck.Episerver.Tests.Utils
{
    public class HCSimpleMemoryCacheForEpiLoadBalancedTests
    {
        [Fact]
        ...
        private static HCSimpleMemoryCacheForEpiLoadBalanced CreateCache(out Mock<IEventRegistry> eventRegistryMock)
        {
            eventRegistryMock = EpiBlobTestHelpers.CreateEventRegistryMock();
            return new HCSimpleMemoryCacheForEpiLoadBalanced(eventRegistryMock.Object);
        }

        private class SimulatedInstance
        {
            public HCSimpleMemoryCacheForEpiLoadBalanced Cache { get; private set; }
