...
{
    public class TKSingleBufferedBlobStorageTests
    {
        public ITestOutputHelper Output { get; }

        public TKSingleBufferedBlobStorageTests(ITestOutputHelper output)
        {
            Output = output;
        }
        ...
        public void InsertBuffered_StoresWhenItShould()
        {
            ...
            var getCounter = 0;
            var setCounter = 0;
            var storage = new TKSingleBufferedBlobStorageTest(cache, Output)
            {
                BlobUpdateBufferDuration = TimeSpan.FromMilliseconds(150),
                MaxBufferSize = 30000,
                Get = () => { getCounter++; return new TKSingleBufferedBlobStorageTest.TestData(); },
                Store = (d) => { setCounter++; }
            };
