using QoDL.Toolkit.Core.Tests.Storage.Implementations;
using System;
using System.Linq;
using Xunit;
using Xunit.Abstractions;

namespace QoDL.Toolkit.Core.Tests.Storage;

public class TKSingleBufferedDictionaryBlobStorageTests
{
    public ITestOutputHelper Output { get; }

    public TKSingleBufferedDictionaryBlobStorageTests(ITestOutputHelper output)
    {
        Output = output;
    }
    ...
    public void Add_StoresValues()
    {
        ...
        var getCounter = 0;
        var setCounter = 0;
        var storage = new TKSingleBufferedDictionaryBlobStorageTest(cache, Output)
        {
            BlobUpdateBufferDuration = TimeSpan.FromDays(1),
            MaxBufferSize = 5,
            Get = () => { getCounter++; return new TKSingleBufferedDictionaryBlobStorageTest.TestData(); },
            Store = (d) => { setCounter++; }
        };
