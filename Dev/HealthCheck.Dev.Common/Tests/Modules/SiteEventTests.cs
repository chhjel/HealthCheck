...
namespace HealthCheck.Dev.Common.Tests.Modules
{
    ...
    public class SiteEventTests
    {
        ...
        {
            HCSiteEventUtils.TryRegisterNewEvent(severity, eventTypeId, title, description, duration, developerDetails,
        ...
        public TestResult AddEventsPreset_Misc1(int count = 500, SiteEventSeverity severity = SiteEventSeverity.Warning)
        {
            for (int i = 0; i < count; i++)
            {
                HCSiteEventUtils.TryRegisterNewEvent(severity, $"pageError_{_eventTime.Ticks}", $"Slow page #{_eventTime.Ticks}",
