...
import { ModuleSpecificConfig } from "@components/ToolkitPageComponent.vue.models";
...
`;
    ...
    saveLocalConfig(): void {
        ...
        localStorage.setItem('__TK_DCE_localOptions', json);
    }

    loadLocalConfig(): LocalOptions {
        const json = localStorage.getItem('__TK_DCE_localOptions');
        if (json != null) {
            try {
    ...
    updateLocalStorage(scripts: Array<LocalOnlyScript>): void {
        ...
        localStorage.setItem('__TK_DCE_localScripts', json);
    }
    ...
    {
        const localScriptsJson = localStorage.getItem('__TK_DCE_localScripts');
    ...
    onNotAllowedModuleSwitch(): void {
        this.confirmUnsavedChanges()
            .then((confirmed) => {
                ...
                {
                    this.code = this.currentScript.Code;
                    setTimeout(() => ((<any>this.$parent)?.$parent as ToolkitPageComponent).retryShowModule(), 50);
                }
            });
    }
}
