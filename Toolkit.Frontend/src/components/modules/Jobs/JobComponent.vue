...
export default class JobComponent extends Vue {
    ...
    pagedHistory: TKPagedJobHistoryEntryViewModel | null = null;
    pagedLogItems: TKPagedJobHistoryEntryViewModel | null = null;
    latestHistory: TKJobHistoryEntryViewModel | null = null;
    latestLogItem: TKJobHistoryEntryViewModel | null = null;
    historyDetails: TKJobHistoryDetailEntryViewModel | null = null;
    historyDetailsParent: TKJobHistoryEntryViewModel | null = null;
    ...
    loadHistoryDetailsFor(history: TKJobHistoryEntryViewModel): void {
    ...
    jobHistoryIcon(history: TKJobHistoryEntryViewModel): string { return JobUtils.jobIcon(null, history); };
    jobHistoryIconColor(history: TKJobHistoryEntryViewModel): string { return JobUtils.jobIconColor(null, history); };

    pagedEntryTitle(entry: TKJobHistoryEntryViewModel): string {
    ...
    detailDialogTitle(entry: TKJobHistoryEntryViewModel): string {
        if (entry.StartedAt != null) {
            const elapsedMs = new Date(entry.EndedAt).getTime() - new Date(entry.StartedAt).getTime();
            const duration = DateUtils.prettifyDurationString(elapsedMs, '', '0 seconds');
    ...
    jobParameterItemClasses(parameter: TKBackendInputConfig): any {
        let classes: any = {
    ...
    get customParameterDefinitions(): TKBackendInputConfig[] { return this.job.Definition.CustomParameters; }
