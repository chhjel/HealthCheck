...
export default class TestParametersComponent extends Vue {
    ...
    allowSmallSize(parameter: TestParameterViewModel): boolean
    {
      ...
      return !parameter.UIHints.includes(TKUIHint.FullWidth)
        && !largerParameters.some(x => parameter.Type == x)
        && !((parameter.Type == 'DateTimeOffset[]' || parameter.Type == 'DateTime[]' || parameter.Type == 'Nullable<DateTimeOffset>[]' || parameter.Type == 'Nullable<DateTime>[]') && parameter.UIHints.includes(TKUIHint.DateRange))
        && !(parameter.Type == 'String' && parameter.UIHints.includes(TKUIHint.TextArea) && parameter.UIHints.includes(TKUIHint.CodeArea));
    }
    ...
    createConfig(parameter: TestParameterViewModel, index: number): TKBackendInputConfig {
      ...
      return {
        ...
        CodeLanguage: parameter.CodeLanguage
      };
    }
