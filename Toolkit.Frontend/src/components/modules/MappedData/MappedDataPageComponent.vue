...
import { TKMappedReferencedTypeDefinitionViewModel } from "@generated/Models/Core/TKMappedReferencedTypeDefinitionViewModel";
import { TKMappedMemberDefinitionViewModel } from "@generated/Models/Core/TKMappedMemberDefinitionViewModel";
...
export default class MappedDataPageComponent extends Vue {
    ...
    definitions: TKMappedDataDefinitionsViewModel = {
        ClassDefinitions: [],
        ReferencedDefinitions: []
    };
    exampleData: Array<TKMappedExampleValueViewModel> = [];
    currentDef: TKMappedClassDefinitionViewModel | null = null;
    ...
    refDefInDialog: TKMappedReferencedTypeDefinitionViewModel | null = null;
    ...
    async mounted()
    {
        ...
        const localStorageOptsJson = localStorage.getItem('__tk_mapping_displayOpts');
    ...
    onDefinitionsRetrieved(data: TKMappedDataDefinitionsViewModel): void {
    ...
    onExamplesRetrieved(data: Array<TKMappedExampleValueViewModel>): void {
    ...
    setActiveType(type: TKMappedClassDefinitionViewModel, updateUrl: boolean = true): void {
    ...
    showRefDefDialog(def: TKMappedReferencedTypeDefinitionViewModel): void {
        this.refDefInDialog = def;
        this.refTypeDialogVisible = true;
    }

    memberPropNameAndDisplayNamesMatches(m: TKMappedMemberDefinitionViewModel): boolean {
        return m.PropertyName == m.DisplayName
            && m.Children.every(c => this.memberPropNameAndDisplayNamesMatches(c));
    }

    memberMappingPropNameAndDisplayNamesMatches(m: TKMappedMemberDefinitionViewModel): boolean {
        return m.MappedTo.every(t => this.memberMappingMatches(t));
    }
    memberMappingMatches(m: TKMappedMemberReferenceDefinitionViewModel): boolean {
    ...
    get exampleDataForCurrentDef(): TKMappedExampleValueViewModel {
    ...
    gotoTypeClicked(def: TKMappedClassDefinitionViewModel, middle: boolean): void {
        if (middle) {
    ...
    onFilterChanged(): void {
        localStorage.setItem('__tk_mapping_displayOpts', JSON.stringify(this.displayOptions));
    }
}
</script>
